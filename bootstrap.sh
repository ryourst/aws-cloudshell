...
istioctl -h &> /dev/null || (echo "Downloading istioctl"; cd ~ && curl --silent -L https://istio.io/downloadIstio | ISTIO_VERSION=1.15.1 TARGET_ARCH=x86_64 sh - && mv "`echo ~`"/istio-?.*.*/bin/istioctl ~/.local/bin/istioctl )
